...
function CartItem( {id ,qty} : cartItem) {
  ...
  const {handleDecreaseQty,handleIncreaseQty,removeProduct} = useShopingCartContext()
    ...
    <div className="mt-8 flex flex-col gap-4 rounded-lg border-b border-blue-300 p-4 shadow-md shadow-blue-700 sm:flex-row">
      ...
      <div className="flex flex-1 flex-col justify-between">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          <Button onClick={()=>{handleIncreaseQty(id)}} size="icon">+</Button>

          <span className="min-w-5 text-center">{qty}</span>

          <Button onClick={()=>{handleDecreaseQty(id)}} size="icon">-</Button>

          <Button onClick={()=>{removeProduct(id)}} size="sm" variant="danger">Remove</Button>
        </div>
      </div>
    </div>
  );
